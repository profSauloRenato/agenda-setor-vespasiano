...
const VersiculosManagerScreen: React.FC = () => {
  ...
  const handleOpenCreateV = () => {
    setVersiculoToEdit(null);
    setVTexto("");
    setVReferencia("");
    setVData(new Date());
    setIsFormVVisible(true);
  };

  const handleOpenEditV = (v: IVersiculo) => {
    setVersiculoToEdit(v);
    setVTexto(v.texto);
    setVReferencia(v.referencia);
    setVData(new Date(v.data + "T12:00:00"));
    setIsFormVVisible(true);
  };

  const handleSaveV = async () => {
    if (!vTexto.trim() || !vReferencia.trim()) {
      Alert.alert("Atenção", "Preencha o texto e a referência.");
      return;
    }
    if (!user) return;
    ...
    try {
      ...
      if (versiculoToEdit) {
        ...
        await versiculoService.createVersiculo(
          { texto: vTexto, referencia: vReferencia, data: dataISO, criado_por: null },
          user.id
        );
      }
    ...
    Alert.alert("Confirmar", "Excluir este versículo?", [
      { text: "Cancelar", style: "cancel" },
      {
        text: "Excluir", style: "destructive", onPress: async () => {
          await versiculoService.deleteVersiculo(id);
          loadVersiculos();
        }
      },
    ]);
  ...
  const handleOpenEditM = (m: IMensagemAdmin) => {
    ...
    setMAtiva(m.ativa);
    setIsFormMVisible(true);
    setMLocalizacaoId(m.localizacao_id);
    setMCargosVisiveis(m.cargos_visiveis);
  };

  const handleSaveM = async () => {
    if (!mTitulo.trim() || !mTexto.trim()) {
    ...
    try {
      if (mensagemToEdit) {
        await mensagemService.updateMensagem(mensagemToEdit.id, {
          titulo: mTitulo,
          texto: mTexto,
          ativa: mAtiva,
      ...
      Alert.alert("Erro", e?.message ?? "Falha ao salvar mensagem.");
    } finally {
  ...
  const handleDeleteM = (id: string) => {
    Alert.alert("Confirmar", "Excluir esta mensagem?", [
      ...
      {
        text: "Excluir", style: "destructive", onPress: async () => {
          await mensagemService.deleteMensagem(id);
          loadMensagens();
        }
      },
    ]);
  ...
  return (
    <SafeScreen style={styles.container}>
      ...
      <View style={styles.abas}>
        <TouchableOpacity
          style={[styles.aba, aba === "versiculos" && styles.abaAtiva]}
          onPress={() => setAba("versiculos")}
        >
          <Text style={[styles.abaText, aba === "versiculos" && styles.abaTextAtiva]}>
            📖 Versículos
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.aba, aba === "mensagens" && styles.abaAtiva]}
          onPress={() => setAba("mensagens")}
        >
          <Text style={[styles.abaText, aba === "mensagens" && styles.abaTextAtiva]}>
            📢 Mensagens
          </Text>
        </TouchableOpacity>
      </View>
        ...
        <View style={styles.content}>
          ...
          {isLoadingV
            ...
            : <FlatList
                ...
                <View style={styles.item}>
                  ...
                  <View style={styles.itemActions}>
                    <TouchableOpacity
                      ...
                      onPress={() => handleOpenEditV(item)}
                    >
                      <Text style={styles.actionBtnText}>Editar</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                      style={[styles.actionBtn, { backgroundColor: "#DC3545", marginTop: 5 }]}
                      onPress={() => handleDeleteV(item.id)}
                    >
              ...
              ListEmptyComponent={
                <Text style={styles.emptyText}>Nenhum versículo cadastrado.</Text>
              }
        ...
        <View style={styles.content}>
          ...
          {isLoadingM
            ...
            : <FlatList
              ...
              renderItem={({ item }) => (
                <View style={styles.item}>
                  ...
                  <View style={styles.itemActions}>
                    <TouchableOpacity
                      style={[styles.actionBtn, { backgroundColor: "#FFC107" }]}
                      onPress={() => handleOpenEditM(item)}
                    >
                      <Text style={styles.actionBtnText}>Editar</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                      style={[styles.actionBtn, { backgroundColor: "#DC3545", marginTop: 5 }]}
                      onPress={() => handleDeleteM(item.id)}
                    >
                      <Text style={styles.actionBtnText}>Excluir</Text>
                    </TouchableOpacity>
                  </View>
              ...
              ListEmptyComponent={
                <Text style={styles.emptyText}>Nenhuma mensagem cadastrada.</Text>
              }
              contentContainerStyle={{ paddingBottom: 20 }}
            />
          }
          ...
          <View style={styles.modalBox}>
            ...
            <ScrollView>
              ...
              <TouchableOpacity style={styles.dateButton} onPress={() => setShowDatePicker(true)}>
                <Text style={styles.dateButtonText}>
                  📅 {vData.toLocaleDateString("pt-BR")}
                </Text>
              </TouchableOpacity>
              {showDatePicker && (
                <DateTimePicker
            ...
            <View style={styles.modalButtons}>
              <TouchableOpacity
                ...
                onPress={() => setIsFormVVisible(false)}
              >
                <Text style={styles.modalBtnText}>Cancelar</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.modalBtn, { backgroundColor: "#17A2B8" }]}
                onPress={handleSaveV}
                disabled={isSubmittingV}
              >
                <Text style={styles.modalBtnText}>
                  {isSubmittingV ? "Salvando..." : "Salvar"}
                </Text>
              </TouchableOpacity>
            </View>
          </View>
      ...
      <Modal visible={isFormMVisible} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            <Text style={styles.modalTitle}>
              {mensagemToEdit ? "Editar Mensagem" : "Nova Mensagem"}
            </Text>
            <ScrollView>
              ...
              <TextInput
                style={[styles.input, styles.textArea]}
                value={mTexto}
                onChangeText={setMTexto}
                placeholder="Ex: Lembramos a todos que..."
                multiline
                numberOfLines={4}
              />
                ...
                {cargosDisponiveis.map((cargo) => {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        setMCargosVisiveis((prev) =>
                          selected ? prev.filter((id) => id !== cargo.id) : [...prev, cargo.id]
                        );
                      }}
                    >
            ...
            <View style={styles.modalButtons}>
              <TouchableOpacity
                style={[styles.modalBtn, { backgroundColor: "#6C757D" }]}
                onPress={() => setIsFormMVisible(false)}
              >
                <Text style={styles.modalBtnText}>Cancelar</Text>
              </TouchableOpacity>
              <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  pickerContainer: { borderWidth: 1, borderColor: "#CED4DA", borderRadius: 8, marginBottom: 5, backgroundColor: "#fff" },
