// supabase/functions/enviar-alertas-eventos/index.ts
...
Deno.serve(async (_req) => {
  try {
    ...
    let totalEnviados = 0;
    const logLinhas: string[] = [`⏰ Iniciando às ${agora.toISOString()}`];

    // ============================================================
    // BLOCO 1 — Alertas de eventos
    // ============================================================
    ...
    logLinhas.push(`📋 Alertas pendentes: ${alertasEventos?.length ?? 0}`);

    for (const alerta of alertasEventos ?? []) {
      const evento = alerta.evento as any;
      if (!evento) { logLinhas.push(`⚠️ Alerta ${alerta.id} sem evento`); continue; }

      const dataEvento = new Date(evento.data_inicio);
      const horaEnvio = new Date(dataEvento.getTime() - alerta.horas_antes * 60 * 60 * 1000);

      logLinhas.push(`🔔 "${evento.titulo}" | horaEnvio: ${horaEnvio.toISOString()} | agora: ${agora.toISOString()} | deve enviar: ${agora >= horaEnvio}`);

      if (agora < horaEnvio) continue;

      const localizacaoIds = await getLocalizacoesFilhas(supabase, evento.abrangencia_id ?? evento.localizacao_id);
      logLinhas.push(`📍 Localizações: ${localizacaoIds.join(", ")}`);
      ...
      logLinhas.push(`👥 Usuários na localização: ${usuarios?.length ?? 0}`);

      if (!usuarios || usuarios.length === 0) continue;

      const cargosVisiveis: string[] = evento.cargos_visiveis ?? [];
      logLinhas.push(`🎯 Cargos visíveis: ${cargosVisiveis.join(", ")}`);
      ...
      logLinhas.push(`✅ Usuários com cargo compatível: ${usuariosFiltrados.length}`);
      ...
      logLinhas.push(`📱 Tokens encontrados: ${tokens?.length ?? 0}`);
      ...
      const messages = tokens.map((t: any) => {
        ...
        return {
          to: t.token,
          title: "🔔 Lembrete!",
          body: `A Paz de Deus, irmão ${nome}! Passando para lembrar de um compromisso na sua agenda: ${evento.titulo} ${formatarData(dataEvento)}. Contamos com sua presença!`,
      ...
      logLinhas.push(`📤 Expo Push response: ${JSON.stringify(pushJson)}`);
      ...
      logLinhas.push(`✅ Alerta ${alerta.id} marcado como enviado`);
    }
    ...
    for (const alerta of alertasCompromissos ?? []) {
      ...
      const horaEnvio = new Date(dataCompromisso.getTime() - alerta.minutos_antes * 60 * 1000);
      if (agora < horaEnvio) continue;

      if (compromisso.recorrente && compromisso.recorrencia_tipo) {
        const hoje = new Date();
        const fim = compromisso.recorrencia_fim ? new Date(compromisso.recorrencia_fim + "T23:59:59") : null;
        ...
        if (compromisso.recorrencia_tipo === "semanal") {
          ...
          if (compromisso.recorrencia_semana_do_mes !== null && compromisso.recorrencia_dia_semana !== null) {
            const semanaDoMes = Math.ceil(hoje.getDate() / 7);
            bate = hoje.getDay() === compromisso.recorrencia_dia_semana && semanaDoMes === compromisso.recorrencia_semana_do_mes;
          } else {
      ...
      const { data: usuario } = await supabase.from("usuario").select("id, nome").eq("id", compromisso.usuario_id).single();
      if (!usuario) continue;

      const { data: tokens } = await supabase.from("usuario_tokens").select("token").eq("usuario_id", compromisso.usuario_id);
      if (!tokens || tokens.length === 0) continue;
      ...
      await supabase.from("compromisso_alerta").update({ enviado: true, enviado_em: new Date().toISOString() }).eq("id", alerta.id);
      totalEnviados += messages.length;
    }
...
async function getLocalizacoesFilhas(supabase: any, localizacaoId: string | null): Promise<string[]> {
  ...
  while (fila.length > 0) {
    ...
    const { data: filhos } = await supabase.from("localizacao").select("id").eq("parent_id", atual);
    for (const filho of filhos ?? []) { ids.push(filho.id); fila.push(filho.id); }
  }
  return ids;
}
...
function formatarData(data: Date): string {
  ...
  return `em ${data.toLocaleDateString("pt-BR", { day: "2-digit", month: "2-digit", year: "numeric", timeZone: tz })} às ${hora}`;
}